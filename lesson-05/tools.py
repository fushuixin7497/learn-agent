...
def flaky_tool(operation: str, fail_rate: float = 0.3) -> str:
    """
    【教学工具】模拟不稳定的外部服务，用于测试 Harness 的重试与恢复机制。
    随机以指定概率返回各种错误（429限流、503不可用、连接超时等）。
    
    参数：
        operation: 要执行的操作描述
        fail_rate: 失败概率，0~1 之间，默认 0.3
    """
    import random

    if not isinstance(operation, str) or not operation:
        return "错误：operation 必须是非空字符串"
    if not isinstance(fail_rate, (int, float)):
        return "错误：fail_rate 必须是数字"
    if fail_rate < 0 or fail_rate > 1:
        return "错误：fail_rate 必须在 0~1 之间"

    if random.random() < fail_rate:
        errors = [
            "错误：请求过于频繁（429 Too Many Requests）。请稍后重试。",
            "错误：服务暂时不可用（503 Service Unavailable）。请稍后重试。",
            "错误：连接超时，无法连接到服务。请检查网络后重试。",
            "错误：网关错误（502 Bad Gateway）。请稍后重试。",
        ]
        return random.choice(errors)

    return f"✅ 成功执行操作：{operation}"
    ...
    tools = [
        ...
        },
        {
            ...
            "function": {
                "name": "flaky_tool",
                "description": (
                    "【教学工具】模拟不稳定的外部服务，随机返回错误。"
                    "用于测试 Agent 的错误恢复和重试机制。"
                    "有 30% 概率返回 429/503/超时等错误。"
                ),
                "parameters": {
                    "type": "object",
                    "properties": {
                        "operation": {
                            "type": "string",
                            "description": "要执行的操作描述，例如 '查询数据库' 或 '发送邮件'",
                        },
                        "fail_rate": {
                            "type": "number",
                            "description": "失败概率，0~1 之间，默认 0.3",
                        },
                    },
                    "required": ["operation"],
                },
